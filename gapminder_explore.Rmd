...
Following [homework prompts here](https://stat545-ubc.github.io/hw04_write-function-split-apply-combine.html)

## First-time setup

Since the output will be in the homework subfolder, this will make knitting easier and place the plots in the right place later

```{r}
suppressPackageStartupMessages(require(knitr))
opts_chunk$set(warning=FALSE, messages=FALSE, cache=TRUE)
```
...
## Writing a function to compute something interesting

### Plotting the slope of each country and it's intercept

Inspired from my peer review [from here](https://github.com/STAT545-UBC/adam_baimel/issues/6#issuecomment-144514497), I will calculate correlation for each country. To quantify variability, I will extract the confidence interval of the slope.
...
It seems like everybody is relatively simiar - what happens if we use a robust linear model?

```{r, error=TRUE}
suppressPackageStartupMessages(require(robustbase))
gapminder_regression_robust <- gapminder %>% 
  group_by(continent, country) %>%
  do(model = lmrob(formula = lifeExp ~ year, data = .)) %>%
  mutate(
    slope = coef(model)[2],
    ylo = confint(model)[2], # confidence interval of the slope
    yhi = confint(model)[4]
...
Looks like some errors were generated - let's try ``MASS::rlm`` as suggested by Dean [here](https://github.com/STAT545-UBC/Discussion/issues/153). I'm using `method == "MM"` as described [here](https://github.com/STAT545-UBC/tony_hui/tree/master/random_class_stuff/stats_crash_course#robust-technique)
...
That worked - perhaps there is better error handling for `rlm` vs `lmrob`. Too lazy to dive into this.

Hmm... no confidence interval estimates for robust linear models... Very weird. Unfortunately, I don't understand enough statistics to figure out why. Anyways, does robust slopes differ from non-robust slopes?
...
slope_diff %>%
  ggplot(aes(slope_diff)) + geom_bar(binwidth=0.02) + geom_vline(xintercept = cutoff) + facet_wrap(~continent)
```
